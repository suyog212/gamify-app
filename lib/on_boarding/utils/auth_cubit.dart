      ...
      if (status.statusCode == 201) {
        ...
        await mixpanel!.track("SignIn", properties: {
          ...
          "Manufacturer": Platform.isAndroid
              ? await DeviceInfoPlugin().androidInfo.then(
                    (value) => value.manufacturer.toString(),
                  )
              : "Apple Inc.",
    ...
    try {
      ...
      if (userData.isNotEmpty) {
        ...
        await Hive.box(userDataDB).put("personalInfo", {
          ...
          "city": data['location'] == null ? "Pune" : data['location'].split(",")[0],
          "state": data['location'] == null ? "Maharashtra" : data['location'].split(",")[1],
          "country": data['location'] == null ? "India" : data['location'].split(",")[2]
        });
        mixpanel!.identify(data['user_id']);
        String interests = data['interests'] ?? "";
        await Hive.box(userDataDB).put("interests", interests != "" ? interests.split(",") : ["Music"]);
