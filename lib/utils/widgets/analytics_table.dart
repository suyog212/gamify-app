...
class _AnalyticsTableState extends State<AnalyticsTable> {
                        ...
                        child: Row(
                          children: [
                            ...
                            SizedBox(
                              width: tileWidth,
                              child: AutoSizeText(
                                  double.parse(widget.data.elementAt(index)['points'].toString()).toStringAsFixed(3)),
                            ),
                            SizedBox(
                              width: tileWidth,
                            ...
                            SizedBox(
                              width: tileWidth,
                              child: AutoSizeText(
                                  "${widget.data.elementAt(index)['total_coins']}"),
                            ),
                            SizedBox(
                              width: tileWidth,
                              child: AutoSizeText(
                                  "${stringToSeconds(widget.data.elementAt(index)['time_taken'])}sec"),
                            ),
                            SizedBox(
                              width: tileWidth,
                              child: AutoSizeText(
                                  "${stringToSeconds(widget.data.elementAt(index)['expected_time'])}sec"),
                            )
                          ],
                        ),
                      ),
                  ...
                  child: DefaultTextStyle(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        SizedBox(
                          width: MediaQuery.sizeOf(context).width * 0.19,
                          child: AutoSizeText(calculateTotal(
                        ...
                        SizedBox(
                          width: MediaQuery.sizeOf(context).width * 0.19,
                          child: AutoSizeText(calculateTotalBonus(
                                  List<Map<String, dynamic>>.from(widget.data))
                              .toStringAsFixed(3)),
                        ),
                        SizedBox(
                        ...
                        SizedBox(
                          width: MediaQuery.sizeOf(context).width * 0.19,
                          child: AutoSizeText(
                              "${calculateTotalSeconds(List<Map<String, dynamic>>.from(widget.data), 'time_taken')}sec"),
                        ),
                        SizedBox(
                          width: MediaQuery.sizeOf(context).width * 0.19,
                          child: AutoSizeText(
                              "${calculateTotalSeconds(List<Map<String, dynamic>>.from(widget.data), 'expected_time')}sec"),
                        )
                      ],
                    ),
                  ),
  ...
  double calculateTotalBonus(List<Map<String, dynamic>> data) {
    ...
    // Iterate through each entry and sum the points
    for (var entry in data) {
      res += ChampionshipScoreEngine().calculateScoreForCorrectAnswer(
          stringToSeconds(entry['expected_time']),
          stringToSeconds(entry['time_taken']),
          double.parse(entry['total_coins'].toString()),
          entry['correct_ans'] == entry['submitted_ans'],
          entry['total_coins'] * -1) - entry['total_coins'];
    }
    return res;
    // double score = ChampionshipScoreEngine()
    //     .calculateScoreForCorrectAnswer(
