  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: RefreshIndicator(
          ...
          onRefresh: () async => context.read<ChampionshipsBloc>().retryFetching() ,
          child: Scrollbar(
            child: CustomScrollView(
              slivers: [
                SliverAppBar(
                  ...
                  elevation: 0,
                  surfaceTintColor: Colors.transparent,
                  title: Text(S.current.championships,style: TextStyle(
                      color: Theme.of(context).colorScheme.secondary,
                      fontWeight: FontWeight.bold
                  ),),
                  bottom: PreferredSize(
                        ...
                        child: Focus(
                          ...
                          child: Padding(
                            ...
                            child: SearchBar(
                              ...
                              onTapOutside: (event) {
                                mixpanel!.track("SearchedForChampionship",properties: {
                        ...
                        builder: (context, snapshot) {
                          if(snapshot.hasData) {
                            return Column(
                              ...
                              children: [
                                CarouselSlider(items: List.generate(snapshot.data!.length, (index) {
                                  return Container(
                                    alignment: Alignment.center,
                                    decoration: BoxDecoration(
                                        borderRadius: BorderRadius.circular(10),
                                        image: DecorationImage(image: CachedNetworkImageProvider(snapshot.data![index]['ad_image']))
                                    ),
                                ...
                                ValueListenableBuilder(valueListenable: dotIndex, builder: (context, value, child) {
                                  return DotsIndicator(dotsCount: snapshot.data!.length,position: value,decorator: DotsDecorator(activeColor: Theme.of(context).colorScheme.secondary),);
                                },),
                              ],
                            );
                          }
                          ...
                          if(snapshot.hasError){
                            return SizedBox(
                              height: MediaQuery.sizeOf(context).height * 0.25,
                              child: const Center(child: AutoSizeText("Error loading...."),),
                            );
                          }
                                                                ...
                                                                Radio(
                                                                  ...
                                                                  onChanged: (value) {
                                                                    setState(() {
                                                                      ...
                                                                      mixpanel!.track("FilterSelected",properties: {
                                                                        "UserId": Hive.box(userDataDB).get("personalInfo")['user_id'],
                                                                        "Filter": value,
                                                                        "timeStamp": DateTime.now()
                                                                      });
                                              ...
                                              if(snapshot.connectionState == ConnectionState.waiting){
                                                return Skeletonizer(
                                                  effect: PulseEffect(
                                                      from: Colors.grey.shade100,
                                                      to: Colors.orange.shade100),
                                                  enabled: true,child: const SizedBox(height: kToolbarHeight,),);
                                              }
                                              if(snapshot.hasData){
