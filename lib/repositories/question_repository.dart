...
class QuestionsRepository {
  ...
  Future<List<QuestionsDetailsModel>?> fetchQuestions(int modeId, userId, int champId) async {
    try {
      await _api.sendRequests.get("/check_user_played.php?user_id=$userId&champ_id=$champId");
  ...
  Future<int> sendQuestionData(int questionId, String timeTaken, String expectedTime, double pointsEarned, String correctAns, int champId, String submittedAns) async {
    try {
  ...
  Future<int> submitChampData(gameMode, totalNegative, champId, totalBonus, totalPenalty, totalScore, timeTaken, expectedTime, userId, totalQuestion, correctQuestion) async {
    try {
